...
export const uploadsController = {
    getCookie(req, res) {
        const uuid = utilsService.createCookieIfMissing(req, res);
    ...
    getDatasets(req, res) {
        ...
        if (!uuid || !signature || !utilsService.verifyUUID(uuid, signature)) {
            return res.status(400).json({ error: 'Invalid or missing UUID/signature' });
        }

        const uploads = utilsService.getUploadsData();
    ...
    getFile(req, res) {
        ...
        const sessionFolder = path.join(utilsService.uploadsFolder, uuid);
    ...
    deleteFile(req, res) {
        ...
        if (uploads[uuid]) {
            ...
            utilsService.saveUploadsData(uploads);
        }
    ...
    upload(req, res) {
        ...
        const userFolder = utilsService.ensureUserFolder(uuid);
        ...
        file.mv(filePath, (err) => {
            ...
            utilsService.updateUploadsJson(uuid, signature, [file.name]);
